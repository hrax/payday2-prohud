if RequiredScript == "lib/managers/hudmanagerpd2" then
	...
	function HUDManager:_create_teammates_panel(hud)
		...
		local playerpanel_height = ((ProHUDOptions.LABEL_HEIGHT + ProHUDOptions.LABEL_GAP) * 2) + ProHUDOptions.PLAYER_ROW -- 110 -- 20 + 5 + 60 + 5 + 20
		local playerpanel_width = (ProHUDOptions.PLAYER_ROW + ProHUDOptions.PANEL_GAP) + ((ProHUDOptions.PLAYER_ROW * 6) + (ProHUDOptions.PANEL_GAP * 2)) + (ProHUDOptions.PLAYER_ROW + ProHUDOptions.PANEL_GAP) + (ProHUDOptions.LABEL_HEIGHT + ProHUDOptions.LABEL_GAP + ProHUDOptions.PLAYER_ROW) -- 605 -- 60 + 5 + 365 + 5 + 60 + 5 + 105
		local playerpanel_gap = 10
		local playerpanel_scale = ProHUDOptions.PLAYER_SCALE
	   
		local teampanel_height = ((ProHUDOptions.LABEL_HEIGHT + ProHUDOptions.LABEL_GAP) * 2) + ProHUDOptions.TEAMMATE_ROW -- 20 + 5 + 50 + 5 + 20
		local teampanel_width = (ProHUDOptions.TEAMMATE_ROW + ProHUDOptions.PANEL_GAP) + ((ProHUDOptions.TEAMMATE_ROW * 6) + (ProHUDOptions.PANEL_GAP)) + (ProHUDOptions.LABEL_HEIGHT + ProHUDOptions.LABEL_GAP + ProHUDOptions.TEAMMATE_ROW) -- 415 -- 50 + 5 + 250 + 5 + 105
		local teampanel_gap = 10
		local teampanel_scale = ProHUDOptions.TEAMMATE_SCALE

		local panel_height = math.max(teampanel_height, playerpanel_height)

		local teammates_panel = hud.panel:panel({
			name = "teammates_panel",
			h = (panel_height + playerpanel_gap) * playerpanel_scale,
			--w = hud.panel:w(),
			y = hud.panel:h() - panel_height,
			halign = "grow",
			valign = "bottom"
		})
		...
		for i = 1, math.max(num_panels, HUDManager.PLAYER_PANEL) do
			...
			local w = is_player and playerpanel_width or teampanel_width
			local h = is_player and playerpanel_height or teampanel_height
			local s = is_player and playerpanel_scale or teampanel_scale
			local g = is_player and playerpanel_gap or teampanel_gap

			local teammate = HUDTeammate:new(i, teammates_panel, is_player, w, h, s)
			local x = math.floor((teammate._panel:w() + g) * (i - 1))
			teammate._panel:set_x(math.min(teammates_panel:w() - teammate._panel:w(), math.floor(x)))
...
elseif RequiredScript == "lib/managers/hud/hudteammate" then
	...
	function HUDTeammate:init(i, teammates_panel, is_player, width, height, scale)
			...
			self:_create_panels(scale)
			--self:_position_panels()

	--[[ 	                if self._main_player then
	            self:_create_player_panel(width)
	        else
	            self:_create_teammate_panel(width)
	        end ]]
	       
	        self:reset_kill_count()
	end

	function HUDTeammate:_create_panels(scale)
		scale = scale or 1

		local panel = self._panel;
		local row_height = self._main_player and ProHUDOptions.PLAYER_ROW or ProHUDOptions.TEAMMATE_ROW
		local label_height = ProHUDOptions.LABEL_HEIGHT * scale
		local middle_top = label_height + ProHUDOptions.LABEL_GAP

		-- create middle row
		self:_create_health_panel(row_height, scale)
		self:_create_weapons_panel(row_height * 6, row_height, scale)

		local eq_w = self._main_player and row_height or ((self._weapons_panel:w() - 5) / 2);
		local eq_h = self._main_player and row_height or label_height
		local eq_s = self._main_player and scale or 1

		self:_create_equipment_panel(eq_w, eq_h, eq_s) -- in case of teammate, this is top row
		self:_create_special_equipment_panel((row_height * scale) + middle_top, 1)

		-- create top row
		self:_create_name_panel((self._health_panel:w() + (self._weapons_panel:w() / 2)), label_height, 1)

		-- create bottom row
		self:_create_latency_panel(self._health_panel:w(), label_height, 1)
		self:_create_kills_panel(self._health_panel:w() * 2, label_height, 1) -- in case of teammate, this is bottom row
		self:_create_carry_panel((self._health_panel:w() + ((self._weapons_panel:w()) / 2)), label_height, 1) -- in case of teammate, this is bottom row

		--ProHUD:debugPanel(panel, Color.blue)
		--ProHUD:debugPanel(self._special_equipment_panel, Color.magneta)

		-- overlays weapons panel
		self:_create_interact_panel_new()

		--ProHUD:debugPanel(self._name_panel, Color.blue)
		--ProHUD:debugPanel(self._equipment_panel, Color.yellow)
		...
		if self._main_player then
			-- place middle panels
			self._health_panel:set_top(middle_top)
			self._weapons_panel:set_top(middle_top)
			self._equipment_panel:set_top(middle_top)
			...
			self._equipment_panel:set_left(self._weapons_panel:right() + ProHUDOptions.PANEL_GAP)
			self._special_equipment_panel:set_left(self._equipment_panel:right() + ProHUDOptions.PANEL_GAP)
	...
	function HUDTeammate:_create_health_panel(side, scale)
		scale = scale or 1
	    width = side * scale
	    height = side * scale
	    ...
	    local function populate_weapon_panel(panel)
	        ...
	        if self._main_player then
	            ...
	            local weapon_selection_bg = weapon_selection_panel:rect({
	                color = Color.white,
	                blend_mode = "normal",
	                h = weapon_selection_panel:h() * math.clamp(2 * 0.25, 0.25, 1),
	                w = weapon_selection_panel:w() * 0.65,
	                visible = false -- for some reason when visible, it overlays the bit maps... why???
	            })
				...
				local firemode_auto = weapon_selection_panel:bitmap({
					...
					firemode_single:set_w(weapon_selection_bg:h() / 2)
					firemode_single:set_h(weapon_selection_bg:h() / 2)
	            firemode_single:set_center(weapon_selection_bg:center())
	            firemode_single:set_top(weapon_selection_bg:top())
	            
	            firemode_auto:set_w(weapon_selection_bg:h() / 2)
					firemode_auto:set_h(weapon_selection_bg:h() / 2)
	            firemode_auto:set_center(weapon_selection_bg:center())
	            firemode_auto:set_bottom(weapon_selection_bg:bottom())
	...
	function HUDTeammate:_create_special_equipment_panel(size, scale)
	    scale = scale or 1
	    width = size * scale
	    height = size * scale
	...
	function HUDTeammate:add_special_equipment(data)
	        local size = self._special_equipment_panel:h() / 3 --(self._main_player and 4 or 3)
