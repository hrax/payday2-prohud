...
local ProHUD_SCREEN_SKIP_STAT_SCREEN_DELAY = -1
local ProHUD_SCREEN_SKIP_SCREEN_DELAY = 3
local ProHUD_SCREEN_SKIP_BLACKSCREEN = -1
local ProHUD_SCREEN_SKIP_AUTO_CARD = 0
...
elseif RequiredScript == "lib/states/ingamewaitingforplayers" then
	...
	function IngameWaitingForPlayersState:update(t, dt)
		if ProHUD_SCREEN_SKIP_BLACKSCREEN == -1 then
			td = 1
		end
		
		ProHUDIngameWaitingForPlayersState_update(self, t, td)
		if self._skip_promt_shown and ProHUD_SCREEN_SKIP_BLACKSCREEN >= 0 then
			self:_skip()
		end
	end
elseif RequiredScript == "lib/states/missionendstate" then
	...
	function MissionEndState.update(self, ...)
		...
		if ProHUD_SCREEN_SKIP_STAT_SCREEN_DELAY == -1 then
